...
class GGMLLayer(torch.nn.Module):
    ...
    def ggml_save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        if self.largest_layer:
            ...
            dtype = self.dequant_dtype if self.dequant_dtype and self.dequant_dtype != "target" else torch.bfloat16
...
class GGMLOps(comfy.ops.manual_cast):
    ...
    class Linear(GGMLLayer, comfy.ops.manual_cast.Linear):
        ...
        def forward_ggml_cast_weights(self, input):
            # 1. Ghi nhớ định dạng gốc của hệ thống (chắc chắn là Float16)
            orig_dtype = input.dtype
            
            # 2. Ép input sang Float32 để chuẩn bị kích hoạt sức mạnh thực sự của GPU
            input_f32 = input.to(torch.float32)
            
            # 3. Yêu cầu ComfyUI-GGUF giải nén Trọng số (Weight) và Bias trực tiếp ra Float32
            weight, bias = self.cast_bias_weight(input, dtype=torch.float32)
            
            # Ép lại bias cho chắc cú (tránh lỗi lọt cặn 16-bit)
            if bias is not None:
                bias = bias.to(torch.float32)
                
            # 4. THỰC HIỆN NHÂN MA TRẬN BẰNG FLOAT32 (Bước này sẽ chạy siêu nhanh ~0.076s)
            result = torch.nn.functional.linear(input_f32, weight, bias)
            
            # 5. Ép kết quả ngược lại thành Float16 trả về cho hệ thống 
            # (Để thằng Attention bên ngoài không bị sảng và không OOM)
    ...
    class Embedding(GGMLLayer, comfy.ops.manual_cast.Embedding):
        def forward_ggml_cast_weights(self, input, out_dtype=None):
            ...
            if self.weight.dtype == torch.bfloat16 or self.weight.dtype == torch.float16:
                out_dtype = None
...
if 'GGMLTensor' in globals():
    _original_tensor_to = torch.Tensor.to
    _shared_pinned_buffer = None

    def _jit_pin_to(self, *args, **kwargs):
        global _shared_pinned_buffer
        ...
        if 'device' in kwargs and 'cuda' in str(kwargs['device']):
            ...
            for arg in args:
                if isinstance(arg, (torch.device, str)) and 'cuda' in str(arg):
                    is_cuda = True
                    break
                    
        if is_cuda and self.device.type == 'cpu':
            try:
                byte_size = self.numel() * self.element_size()
                if _shared_pinned_buffer is None or _shared_pinned_buffer.numel() < byte_size:
                    _shared_pinned_buffer = torch.empty(byte_size, dtype=torch.uint8, pin_memory=True)
                view = _shared_pinned_buffer[:byte_size].view(self.dtype).reshape(self.shape)
                view.copy_(self)
                res = _original_tensor_to(view, *args, **kwargs)
                if hasattr(self, "tensor_type"):
                    res.tensor_type = self.tensor_type
                if hasattr(self, "tensor_shape"):
                    res.tensor_shape = self.tensor_shape
                return res
            except Exception:
                pass
                
        res = _original_tensor_to(self, *args, **kwargs)
        if hasattr(self, "tensor_type"):
            res.tensor_type = self.tensor_type
        if hasattr(self, "tensor_shape"):
            res.tensor_shape = self.tensor_shape
        return res
    GGMLTensor.to = _jit_pin_to
