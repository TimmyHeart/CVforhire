...
def hacked_sdpa(query, key, value, *args, **kwargs):
    if query.dtype == torch.bfloat16:
        # Lột vỏ (Sẽ tốn VRAM cục bộ)
        q_f32 = query.to(torch.float32)
        k_f32 = key.to(torch.float32)
        v_f32 = value.to(torch.float32)
        
        # Tính toán Max tốc độ
        out = original_sdpa(q_f32, k_f32, v_f32, *args, **kwargs)
        
        # SÁT THỦ VRAM: Giết ngay lập tức các bản copy F32 để chống tràn!
        del q_f32, k_f32, v_f32 
        
        # Trả lại vỏ BF16 và giết nốt bản F32 out
        res = out.to(torch.bfloat16)
        del out
        return res
        
    return original_sdpa(query, key, value, *args, **kwargs)

F.scaled_dot_product_attention = hacked_sdpa

# Áp dụng y hệt luật "Giết xác F32" cho Xformers
try:
    import xformers.ops
    original_xformers_mea = xformers.ops.memory_efficient_attention
    
    def hacked_xformers_mea(query, key, value, *args, **kwargs):
        if query.dtype == torch.bfloat16:
            q_f32 = query.to(torch.float32)
            k_f32 = key.to(torch.float32)
            v_f32 = value.to(torch.float32)
            
            out = original_xformers_mea(q_f32, k_f32, v_f32, *args, **kwargs)
            ...
            del out
            return res
        return original_xformers_mea(query, key, value, *args, **kwargs)
        
    xformers.ops.memory_efficient_attention = hacked_xformers_mea
except ImportError:
    pass
# ====================================================================
...
def should_use_fp16(device=None, model_params=0, prioritize_performance=True, manual_cast=False):
    if prioritize_performance and not manual_cast:
        return False
