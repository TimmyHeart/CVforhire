...
def attention_xformers(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False, **kwargs):
    ...
    if SDP_BATCH_LIMIT >= b:
        ...
        m_f32 = mask.to(torch.float32) if mask is not None else None
        out = xformers.ops.memory_efficient_attention(q_f32, k_f32, v_f32, attn_bias=m_f32)
        ...
        for i in range(0, b, SDP_BATCH_LIMIT):
            ...
            if mask is not None:
                if mask.shape[0] > 1:
                    m_slice = mask[i : i + SDP_BATCH_LIMIT]
                m_slice = m_slice.to(torch.float32)
            q_slice = q[i : i + SDP_BATCH_LIMIT].to(torch.float32)
            k_slice = k[i : i + SDP_BATCH_LIMIT].to(torch.float32)
            v_slice = v[i : i + SDP_BATCH_LIMIT].to(torch.float32)
            out_slice = xformers.ops.memory_efficient_attention(
                q_slice, k_slice, v_slice, attn_bias=m_slice
            )
            out[i : i + SDP_BATCH_LIMIT] = out_slice.to(orig_dtype)
...
def attention_pytorch(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False, **kwargs):
    ...
    if SDP_BATCH_LIMIT >= b:
        ...
        for i in range(0, b, SDP_BATCH_LIMIT):
            ...
            q_slice_f32 = q[i : i + SDP_BATCH_LIMIT].to(torch.float32)
            k_slice_f32 = k[i : i + SDP_BATCH_LIMIT].to(torch.float32)
            v_slice_f32 = v[i : i + SDP_BATCH_LIMIT].to(torch.float32)
            m_f32 = m.to(torch.float32) if m is not None else None
            out_slice_f32 = comfy.ops.scaled_dot_product_attention(
                q_slice_f32,
                k_slice_f32,
                v_slice_f32,
                attn_mask=m_f32,
            ...
            out[i : i + SDP_BATCH_LIMIT] = out_slice_f32.to(orig_dtype).transpose(1, 2).reshape(-1, q.shape[2], heads * dim_head)
    return out
...
def attention_flash(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False, **kwargs):
    ...
    try:
        ...
        logging.warning(f"Flash Attention failed, using HACKED SDPA Float32: {e}")
        orig_dtype = q.dtype
        q_f32, k_f32, v_f32 = q.to(torch.float32), k.to(torch.float32), v.to(torch.float32)
        mask_f32 = mask.to(torch.float32) if mask is not None else None
